...
Status: container mapped, palette locator solved (Stage 1). Level-data format
still undecoded (Stage 2).
...
- The level-data format: how the playfield, scroll, and enemy spawns are
  encoded, and whether they live in the WAD's data segment or in the per-level
  `.BIN` (`out.bin`, `race1.bin`, ...). Each WAD also has a large run of
  `<= 0x3F` bytes (roughly 6.4 to 8 KB) separate from the palette; it is a
  candidate for level/tile data and worth a look first.
