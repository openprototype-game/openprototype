//! Decode CD-DA PCM bytes into normalized `f32` samples.
//!
...
/// Convert raw little-endian 16-bit PCM into `f32` in `[-1.0, 1.0)`.
...
/// Append `bytes` (little-endian 16-bit PCM) to `out`, normalized to
/// `[-1.0, 1.0)`. Lets a streaming source refill its buffer without
/// reallocating.
