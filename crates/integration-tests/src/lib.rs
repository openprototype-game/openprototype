...
/// Locates a cue.
///
/// `$PROTOTYPE_DISC` first, then `PROTOTYPE.cue` at the repo root (two levels
/// up from this crate).
...
/// Opens the disc image.
///
/// Callers are the `disc-tests`-gated suite, so reaching this without an image
...
/// Canonical names of every file whose name ends with `ext`.
///
/// Case-insensitive (e.g. `".FLI"`), sorted for deterministic iteration.
